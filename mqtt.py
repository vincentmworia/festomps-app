...
def on_message(client_msg, userdata, msg):
    ...
    if topic == 'REQUEST INIT' and payload == 'b\'true\'':
        publish_data("CODE STEP DISTRIBUTION", dist_code_step() + ':unknown')
        ...
        publish_data("CODE STEP SORTING", sort_code_step() + ':' + sort_workpiece())
...
def stream_code_step_dist():
    ...
    while True:
        ...
        if old_dist_code_step != new_dist_code_step:
            publish_data("CODE STEP DISTRIBUTION", new_dist_code_step + ':unknown')
...
def stream_code_step_sort():
    ...
    while True:
        ...
        if old_sort_code_step != new_sort_code_step:
            publish_data("CODE STEP SORTING", new_sort_code_step + ':' + sort_workpiece())
...
def stream_workpiece_number_sort():
    old_sort_workpiece_number = ''
    while True:
        new_sort_workpiece_number = sort_workpiece_total()
        if old_sort_workpiece_number != new_sort_workpiece_number:
            publish_data("WORKPIECE COUNT", sort_workpiece_black() +
                         ':' + sort_workpiece_metallic() +
                         ':' + sort_workpiece_red() +
                         ':' + sort_workpiece_total())
            old_sort_system_on = new_sort_workpiece_number
        time.sleep(update_time)
